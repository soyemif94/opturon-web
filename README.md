...
Requiere definir antes:
- `SAAS_STAFF_EMAIL`
- `SAAS_STAFF_PASSWORD`
- `SAAS_CLIENT_OWNER_EMAIL`
- `SAAS_CLIENT_OWNER_PASSWORD`

El script falla de forma explicita si falta alguno de esos env vars.
...
```bash
AUTH_EMAIL=tu-admin@opturon.com AUTH_PASSWORD=tu-password npm run auth:smoke
AUTH_BASE_URL=https://opturon.com AUTH_EMAIL=tu-admin@opturon.com AUTH_PASSWORD=tu-password npm run auth:smoke
```

### PowerShell (Windows)
```powershell
$env:AUTH_EMAIL="tu-admin@opturon.com"
$env:AUTH_PASSWORD="tu-password"
npm run auth:smoke
```

## E2E / Playwright
- Playwright no corre desde `dev`, `build` ni `start`.
- La entrada explicita es `npm run e2e:playwright`.
- El runner exige `ALLOW_E2E=true`, `TEST_USER_EMAIL` y `TEST_USER_PASSWORD`.
...
- `BACKEND_BASE_URL=` (vacï¿½o para modo interno)
