...
export async function GET(req: Request) {
  const sha = process.env.VERCEL_GIT_COMMIT_SHA ?? process.env.NEXT_PUBLIC_APP_BUILD_SHA ?? null;
  ...
  const branch = process.env.VERCEL_GIT_COMMIT_REF ?? process.env.NEXT_PUBLIC_APP_BUILD_BRANCH ?? null;
