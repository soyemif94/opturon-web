...
import { getBackendErrorStatus, isBackendConfigured, previewPortalWhatsAppChatImport } from "@/lib/api";
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const result = await previewPortalWhatsAppChatImport(
    ...
    return noStore(
      NextResponse.json(
        { error: error instanceof Error ? error.message : "whatsapp_import_preview_failed" },
