...
import { getBackendErrorBody, getBackendErrorStatus, isBackendConfigured, updatePortalInventoryLotExpiration } from "@/lib/api";
...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ lotId: string }> }) {
  ...
  try {
    ...
    const result = await updatePortalInventoryLotExpiration(tenantContext.tenantId, lotId, {
