...
export async function POST(_request: NextRequest, { params }: { params: Promise<{ importId: string }> }) {
  ...
  try {
    const result = await cancelPortalCatalogImport(tenantContext.tenantId, importId, actorFromTenantContext(tenantContext));
    return noStore(NextResponse.json({ ok: true, import: result.data }));
  } catch (error) {
    return noStore(
      NextResponse.json(
        {
          error: error instanceof Error ? error.message : "catalog_import_cancel_failed"
        },
