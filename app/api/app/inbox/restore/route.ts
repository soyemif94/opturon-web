...
import { getBackendErrorBody, getBackendErrorStatus, isBackendConfigured, restorePortalConversations } from "@/lib/api";
...
export async function PATCH(request: NextRequest) {
  ...
  try {
    ...
    const result = await restorePortalConversations(tenantContext.tenantId, conversationIds);
    ...
    return noStore(
      NextResponse.json(
        backendBody && typeof backendBody === "object"
          ? backendBody
          : { error: error instanceof Error ? error.message : "inbox_restore_failed" },
