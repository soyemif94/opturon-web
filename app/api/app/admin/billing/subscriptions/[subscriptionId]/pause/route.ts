...
export async function POST(_request: NextRequest, { params }: { params: Promise<{ subscriptionId: string }> }) {
  ...
  try {
    const result = await postAdminBillingSubscriptionAction(subscriptionId, "pause");
    return noStore(NextResponse.json(result.data));
  } catch (error) {
    return noStore(
      NextResponse.json(
        getBackendErrorBody(error) || {
          error: "admin_billing_subscription_pause_failed",
          detail: error instanceof Error ? error.message : "No se pudo pausar la suscripcion."
        },
