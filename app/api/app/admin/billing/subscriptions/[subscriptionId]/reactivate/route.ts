...
export async function POST(_request: NextRequest, { params }: { params: Promise<{ subscriptionId: string }> }) {
  ...
  try {
    const result = await postAdminBillingSubscriptionAction(subscriptionId, "reactivate");
    return noStore(NextResponse.json(result.data));
  } catch (error) {
    return noStore(
      NextResponse.json(
        getBackendErrorBody(error) || {
          error: "admin_billing_subscription_reactivate_failed",
          detail: error instanceof Error ? error.message : "No se pudo reactivar la suscripcion."
        },
