...
export async function GET(
  ...
  try {
    const result = await requestAdminTenantOperationalAlerts<{ success: boolean; data: unknown }>(
      ...
      targetTenantId,
      `/rules/${encodeURIComponent(safeRuleId)}/candidate-preview`,
      { actorUserId }
    );
    ...
    return noStore(
      NextResponse.json(
        getBackendErrorBody(error) || {
          error: "admin_operational_alert_candidate_preview_failed",
          detail: error instanceof Error ? error.message : "No se pudo obtener el preview."
        },
