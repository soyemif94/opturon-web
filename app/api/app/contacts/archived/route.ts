...
import { deletePortalArchivedContacts, getBackendErrorBody, getBackendErrorStatus, isBackendConfigured } from "@/lib/api";
...
export async function DELETE(request: NextRequest) {
  ...
  try {
    ...
    const result = await deletePortalArchivedContacts(tenantContext.tenantId, contactIds);
    ...
    return noStore(
      NextResponse.json(
        backendBody && typeof backendBody === "object"
          ? backendBody
          : {
              error: error instanceof Error ? error.message : "contacts_delete_failed"
            },
