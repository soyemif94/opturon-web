...
import { getBackendErrorBody, getBackendErrorStatus, getPortalPaymentDetail, isBackendConfigured } from "@/lib/api";
...
export async function GET(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    const result = await getPortalPaymentDetail(tenantContext.tenantId, id);
    return noStore(NextResponse.json({ tenantId: tenantContext.tenantId, payment: result.data }));
  } catch (error) {
