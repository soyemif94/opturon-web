...
export async function POST(request: NextRequest, context: { params: Promise<{ applicationId: string }> }) {
  ...
  try {
    const result = await callPartnerRecruitmentBackend(
      `/api/partners/me/recruitment-applications/${encodeURIComponent(applicationId)}/reopen-for-edit`,
      { method: "POST", body: "{}" },
    ...
    return noStore(
      NextResponse.json(
        getRecruitmentBackendErrorBody(error) || { error: "reopen_for_edit_failed" },
