...
export default async function AppIntegrationsPage() {
  ...
  if (ctx.tenantId && isBackendConfigured()) {
    try {
      const [result, onboarding, statusResult, blueprintsResult, templatesResult] = await Promise.all([
  ...
  return (
    ...
    >
      <IntegrationsHub whatsapp={whatsapp} whatsappStatus={whatsappStatus} templateBlueprints={templateBlueprints} templates={templates} />
    </ClientPageShell>
  );
}
