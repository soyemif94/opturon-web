...
export function getClientRequestBackendErrorStatus(error: unknown) {
...
export function getClientRequestBackendErrorBody(error: unknown) {
  if (error && typeof error === "object" && "body" in error) return (error as BackendError).body;
  return undefined;
}

export async function callClientRequestBackend<T>(
  path: string,
  init: RequestInit = {},
  actor: BackendActor = {},
...
export async function fetchClientRequestReceipt(path: string, actor: BackendActor) {
  const apiBase = getApiBaseUrl();
  if (!apiBase) throw new Error("API base URL is not configured");
  const portalKey = getPortalInternalKey();
  if (!portalKey) throw new Error("PORTAL_INTERNAL_KEY is not configured");
  ...
  if (!response.ok) {
    ...
    try {
      ...
      body = { error: "receipt_fetch_failed" };
    }
    throw createBackendError(response.status, body, `API request failed (${response.status})`);
  }
  return response;
}
